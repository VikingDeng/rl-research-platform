...
export const JobQueue: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">{tx('集群与队列', 'Cluster & Queue')}</h1>
        <p className="text-gray-500 mt-1">
          {tx('监控 GPU 资源并管理作业调度。', 'Monitor GPU resources and manage job scheduling.')}
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-gray-900 flex items-center">
                <Server className="w-5 h-5 mr-2 text-gray-500" /> {tx('活跃运行', 'Active Runs')}
            </h2>
            <span className="px-3 py-1 bg-green-50 text-green-700 rounded-full text-xs font-medium border border-green-200">
                {tx(`${activeRuns.length} 个运行中`, `${activeRuns.length} running`)}
            </span>
        </div>
            ...
            {activeRuns.map(run => (
              <div key={run.id} className="p-4 rounded-lg border border-gray-200 bg-gray-50">
                  ...
                  <div className="mt-3 flex items-center gap-2 text-xs text-gray-600">
                      <Cpu className="w-3 h-3" /> {tx(`${run.gpu ?? 0} 张 GPU`, `${run.gpu ?? 0} GPU`)}
                  </div>
              </div>
            ))}
            {activeRuns.length === 0 && (
              <div className="col-span-full h-32 flex flex-col items-center justify-center text-gray-400 bg-gray-50 rounded-lg border border-dashed border-gray-200">
                  ...
                  <span className="text-sm">{tx('暂无活跃运行', 'No active runs')}</span>
              </div>
            )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
             <h2 className="text-lg font-semibold text-gray-900 flex items-center">
                <Clock className="w-5 h-5 mr-2 text-gray-500" /> {tx('待调度作业', 'Pending Jobs')}
            </h2>
            <span className="text-sm text-gray-500">
              {tx(`${pendingRuns.length} 个作业等待`, `${pendingRuns.length} jobs waiting`)}
            </span>
        </div>
        <table className="w-full text-left">
            <thead className="bg-gray-50 text-xs text-gray-500 font-semibold uppercase tracking-wider">
                <tr>
                    <th className="px-6 py-3">{tx('排名', 'Rank')}</th>
                    <th className="px-6 py-3">{tx('运行', 'Run')}</th>
                    <th className="px-6 py-3">{tx('类型', 'Type')}</th>
                    <th className="px-6 py-3">{tx('资源', 'Resources')}</th>
                ...
                {pendingRuns.map((run, idx) => (
                    <tr key={run.id} className="hover:bg-gray-50 group">
                        ...
                        <td className="px-6 py-4 text-sm text-gray-600">
                          {tx(`${run.gpu ?? 0} 张 GPU`, `${run.gpu ?? 0} GPU`)}
                        </td>
                        <td className="px-6 py-4 text-sm text-gray-500">
                          {new Date(run.created).toLocaleString(locale === 'zh' ? 'zh-CN' : 'en-US')}
                        </td>
                    </tr>
                ))}
