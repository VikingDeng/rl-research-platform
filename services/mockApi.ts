...
const seedAgenticShowcaseRun = () => {
  ...
  for (let round = 0; round < 14; round += 1) {
    ...
    detail.llmTraces.push({
      ...
      promptTokens: 260 + Math.round(Math.random() * 80),
      completionTokens: 90 + Math.round(Math.random() * 40),
      nodeId: node.nodeId,
    ...
    if ((node.children || []).length === 0 && depth < 3 && detail.totTree.length < 28) {
      ...
      detail.llmTraces.push({
        ...
        promptTokens: 320 + Math.round(Math.random() * 120),
        completionTokens: 140 + Math.round(Math.random() * 60),
        nodeId: node.nodeId,
